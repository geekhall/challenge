...
export default class BlasterScene extends THREE.Scene {
  ...
  async initialize() {
    // load a shared MTL(Material Template Library) for the target.
  ...
  private updateInput() {
    ...
    const upDownSpeed = 0.1
    const leftRightspeed = 0.1

    if (this.keyDown.has('w') || this.keyDown.has('arrowup'))
      this.blaster.position.add(dir.clone().multiplyScalar(upDownSpeed))
    ...
    if (shiftKey) {
      ...
      if (this.keyDown.has('a') || this.keyDown.has('arrowleft')) {
        this.blaster.position.add(
          strafeDir.applyAxisAngle(upVector, Math.PI / 2).multiplyScalar(leftRightspeed),
        )
      }
      else if (this.keyDown.has('d') || this.keyDown.has('arrowright')) {
        this.blaster.position.add(
  ...
  getACube() {
    ...
    const material = new THREE.MeshPhongMaterial({ color: 0x69AD93 })
